...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# write a new model, lm4 for comparison 
lm2 = lm(price ~ . - pctCollege - sewer - waterfront - landValue - newConstruction, data=saratoga_train)
lm3 = lm(price ~ (. - pctCollege - sewer - waterfront - landValue - newConstruction)^2, data=saratoga_train)
lm4 = lm(price ~ . - pctCollege - sewer - fuel -fireplaces -livingArea -landValue +poly(landValue, 2) +loglandValue +livingArea:centralAir +bathrooms:heating, data=saratoga_train)
```
...
credit_table <- data.frame(addmargins(prop.table(table(credit$Default,
                                            credit$history,
                                            dnn=c('Default', 'credit_history'))))) # make a df on defaulted credit account
# filter
default_prop <- credit_table %>%
  filter(Default == 1 & credit_history != 'Sum')
# make the bar plot
ggplot(default_prop, aes(x= credit_history, y=Freq))+
